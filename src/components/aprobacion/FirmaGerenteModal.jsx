...
export default function FirmaGerenteModal({ open, onClose, onFirmada, user, displayName: displayNameProp, tipoCertificado, firmaUrl: firmaUrlProp }) {
  ...
  // Cargar firma guardada del empleado — búsqueda robusta por user_id primero
  // (igual que AuthContext), luego por email con match case-insensitive exacto
  const { data: empleados = [], isLoading: loadingFirma } = useQuery({
    queryKey: ['employee-firma-gerente', user?.id, user?.email],
    queryFn: async () => {
      if (!user) return [];
      if (user.id) {
        try {
