...
import { Download, Copy, Check, ClipboardList, Clock, CheckCircle2, AlertCircle, Loader2, Cpu } from 'lucide-react';

/**
 * Deriva la URL base de la app desde el pathname actual.
 * Ej: /apps/myapp/activos → /apps/myapp  |  /activos → (vacío)
 */
...
/**
 * Modal de QR para un Activo — espejo de LocationQRModal.
 * El QR codifica: {origin}{appBase}/portal-operario?asset={assetId}
 * El operario lo escanea → ve las OTs vinculadas al activo → las ejecuta.
 */
export default function AssetQRModal({ open, onOpenChange, asset }) {
  ...
  const qrValue = asset ? `${window.location.origin}${getAppBaseUrl()}/portal-operario?asset=${asset.id}` : '';

  // Cargar OTs del activo vía backend (service role, sin RLS)
  useEffect(() => {
    if (!open || !asset) return;
    let cancelled = false;
    setIsLoadingOTs(true);
    setWorkOrders([]);
    base44.functions.invoke('publicFichar', { action: 'getWorkOrdersForAsset', assetId: asset.id })
      .then(res => { if (!cancelled) setWorkOrders(res.data?.workOrders || []); })
      .catch(() => { if (!cancelled) setWorkOrders([]); })
      .finally(() => { if (!cancelled) setIsLoadingOTs(false); });
  ...
  const handleDownload = () => {
    ...
    link.download = `QR_Activo_${(asset?.name || 'codigo').replace(/\s+/g, '_')}.png`;
      ...
      <DialogContent className="max-w-md max-h-[85vh] overflow-y-auto">
        ...
        <div className="flex flex-col items-center gap-3 py-1">
          <div className="bg-white border-2 border-border rounded-2xl p-4 shadow-sm flex flex-col items-center gap-2">
            <p className="font-bold text-sm text-center text-foreground leading-tight">{asset.name}</p>
            {asset.code && <p className="text-xs text-muted-foreground text-center font-mono">{asset.code}</p>}
            {(asset.sede || asset.location) && <p className="text-xs text-muted-foreground text-center">{asset.sede || asset.location}</p>}
          ...
          <p className="text-[11px] text-muted-foreground text-center max-w-[280px]">
            El operario escanea este QR para ver las OTs del activo y ejecutarlas.
          </p>
        ...
        <div className="border-t border-border pt-3">
          ...
          {isLoadingOTs ? (
            ...
            <div className="text-center py-6 text-xs text-muted-foreground">
              No hay OTs generadas para este activo
            </div>
          ) : (
