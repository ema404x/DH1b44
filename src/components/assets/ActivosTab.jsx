  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <Button onClick={openNew} className="gap-1.5"><Plus className="h-4 w-4" /> Nuevo Activo</Button>
      </div>
                ...
                <div className="space-y-1.5 text-xs text-muted-foreground">
                  ...
                  {maintStatus && (
                    <div className={`flex items-center gap-1.5 font-medium ${maintStatus.color}`}>
                      <Clock className="h-3 w-3" />Próx. mantenimiento: {maintStatus.label}
                    </div>
                  )}
                </div>
                <div className="flex items-center justify-between mt-3 pt-2.5 border-t">
                  <Badge className={`text-xs ${critColors[asset.criticality]}`}>Criticidad {asset.criticality}</Badge>
      ...
      <QRCodeModal open={!!qrAsset} onClose={() => setQrAsset(null)} title={qrAsset?.name || ''} subtitle={`${qrAsset?.code ? `Cód: ${qrAsset.code} · ` : ''}${qrAsset?.location || ''}`} value={qrAsset ? `${window.location.origin}/activos?id=${qrAsset.id}&code=${qrAsset.code || ''}` : ''} />

      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
          <DialogHeader><DialogTitle>{editing ? `${editing.name}` : 'Nuevo Activo'}</DialogTitle></DialogHeader>
            ...
            <TabsContent value="datos" className="flex-1 overflow-y-auto mt-3">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5"><Label className="text-xs">Criticidad</Label>
                  <Select value={form.criticality} onValueChange={v => set('criticality', v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="baja">Baja</SelectItem><SelectItem value="media">Media</SelectItem><SelectItem value="alta">Alta</SelectItem><SelectItem value="critica">Crítica</SelectItem></SelectContent></Select>
                </div>
