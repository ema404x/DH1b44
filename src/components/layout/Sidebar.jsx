...
export default function Sidebar({ open, onOpenChange }) {
  ...
  // Drawer mobile simplificado: solo General, Emergencias y Operaciones.
  // Mismos permisos que el sidebar desktop, filtrando a las 3 secciones operativas.
  const mobileVisibleGroups = useMemo(() => {
    const KEEP = new Set(['General', 'Emergencias', 'Operaciones']);
    return visibleGroups.filter(g => KEEP.has(g.label));
  }, [visibleGroups]);
    ...
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 py-3 px-2 overflow-y-auto space-y-5 scrollbar-thin">
        {mobileVisibleGroups.map((group) => (
          <div key={group.label}>
            <div className="px-3 mb-1.5 mt-1 text-[10px] font-bold uppercase tracking-[0.16em] flex items-center gap-2 text-sidebar-foreground/70">
              <span className="flex-shrink-0">{group.label}</span>
              <div className="flex-1 h-px bg-border/40" />
              <span className="text-[9px] text-sidebar-foreground/50 tabular-nums shrink-0">{group.items.length}</span>
            </div>
