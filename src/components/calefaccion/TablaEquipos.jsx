...
export default function TablaEquipos({ equipos, estadoConfig, tipoLabels }) {
  ...
  for (const e of equipos) {
    const key = e.escuela;
    if (!grupoMap[key]) {
      grupoMap[key] = { escuela: e.escuela, jefe_sitio: e.jefe_sitio, comuna: e.comuna, registros: [] };
    }
    grupoMap[key].registros.push(e);
  }
  const grupos = Object.values(grupoMap);

  const toggle = (escuela) => setExpandidas(prev => ({ ...prev, [escuela]: !prev[escuela] }));
            ...
            {grupos.map((g) => {
              ...
              return (
                <React.Fragment key={g.escuela}>
                  ...
                  >
                    <td className="px-4 py-3 text-white text-xs font-semibold max-w-[200px] truncate">{g.escuela}</td>
                    <td className="px-4 py-3 text-slate-300 text-xs max-w-[140px] truncate hidden sm:table-cell">{g.jefe_sitio || '—'}</td>
                    ...
                    <td className="px-3 py-3 text-center">
                      <Badge className={`text-[10px] px-2 py-0.5 border ${cfg.badge}`}>{cfg.label}</Badge>
                    </td>
                  ...
                  {expanded && g.registros.map((e, idx) => {
                    ...
                    return (
                      <tr key={e.id || idx} className="border-b border-slate-700/20 bg-slate-900/40 hover:bg-slate-800/20 transition-colors">
                        <td className="pl-8 pr-4 py-2 text-slate-400 text-xs italic">
                        ...
                        <td className="px-3 py-2 text-center">
                          <Badge className={`text-[9px] px-1.5 py-0 border ${ecfg.badge}`}>{ecfg.label}</Badge>
                        </td>
                        <td />
